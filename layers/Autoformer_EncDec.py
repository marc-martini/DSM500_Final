...
class FEDDecoderLayer(nn.Module):
    ...
    """
    def __init__(self, self_attention, cross_attention, d_model, c_out, d_ff=None,
                 moving_avg=25, dropout=0.1, activation="relu", seq_len=None, patch_len=None, stride=None, padding_patch=None, enc_in=None, head=None):
        ...
        self.head = head
        self.n_vars = enc_in
        self.model_proj = nn.Linear(self.n_vars, d_model)
    ...
    def forward(self, x, cross, x_mask=None, cross_mask=None):
        ...
        x = x.permute(0,1,3,2)                                                              # x_enc: [bs x nvars x patch_len x patch_num]
        


        n_vars = x.shape[1]
        # Project x_enc into the model dimension space 
        x = x.permute(0,1,3,2)                                                              # x_enc: [bs x nvars x patch_num x patch_len]

        x = self.W_P(x)                                                                     # x_enc: [bs x nvars x patch_num x d_model]

        # channel independece for x_enc
        x = torch.reshape(x, (x.shape[0]*x.shape[1],x.shape[2],x.shape[3]))         # x_enc: [bs * nvars x patch_num x d_model]
        ...
        x = x.permute(0,1,3,2)                                                               # enc_out: [bs x nvars x d_model x patch_num]

        # apply flatten head
        x = self.head(x)                                                                    # enc_out: [bs x nvars x target_window] 
